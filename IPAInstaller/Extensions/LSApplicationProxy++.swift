...
extension LSApplicationProxy: Hashable {
    ...
    var accessibleContainers: [String] {
        ...
        if !self.hasUnrestrictedContainerAccess {
            let bundleID = self.getValueForEntitlement("CFBundleIdentifier")
            guard bundleID as? String != "" && type(of: bundleID) == String.self else {
                print("ERROR: CFBundleIdentifier was not expected type")
                return [String]()
            }
            containersToReturn.append(bundleID as! String)
        ...
        if self.hasEntitlement("com.apple.private.tcc.allow") {
            if let services = self.getValueForEntitlement("com.apple.private.tcc.allow") as? [String] {
                for service in services {
                    let commonTCCServices: [String:String] = [
